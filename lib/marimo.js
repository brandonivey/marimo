...
marimo.register_module = function register_module (module) {
  // if the namespace is already used fail fast
  if (marimo[module.name]) {
    console.log('Failed to register module "' + module.name + '" because there is already a module of that name');
  };
  marimo[module.name] = module
  for (widgetname in module.widget_lib){
    if (!module.widget_lib.hasOwnProperty(widgetname)) continue
    if (marimo.widget_lib[widgetname]) {
      console.log('Failed to add widget "' + widgetname + '" from module "' + module.name + '" because there is already a widget of that name');
      continue;
    }
    // in that case it's safe to add the module
    marimo.widget_lib[widgetname] = module.widget_lib[widgetname];
  }
  // a hook for any other setup that might be required
  if(module.register) {
    module.register(marimo);
  }
};
