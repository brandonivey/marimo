...
var marimo = {
    /* this namespace stores the general marimo state (widget requests, live
       ...
       extended.
    */
	// maps target URLs to batch-request objects that widgets use to get fresh
	// data about themselves.
    requests: {},
	// maps widget ids to live widgets on the page
    widgets: {},
    // an event registry for tracking events that have occurred. Maps 
    events: {},
    init: function init($) {
...
marimo.batch_request = {
    ...
    */
    ...
    make_request: function make_request(cb) {
        ...
        marimo.$.ajax({
            url: this.url,
            data: {bulk:JSON.stringify(payloads)},
            dataType: 'json',
